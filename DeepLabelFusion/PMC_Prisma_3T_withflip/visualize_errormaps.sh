...
TEMPLATEDIR=$EXPROOT/template_all
VOLUMEDIR=$EXPROOT/refseg_volume_3T
RAWDATADIR=$ROOT/data
#RAWDATADIR=/data/picsl/lwisse/NACC_Prisma_Atlas_NotFinal
AUTOSEGDIR=$TEMPLATEDIR/autoseg_fold1-3
...
LABEL_MRG_ALL=("0 6 7 13" 1   2   3  4   5    8   9   10   11   12  14)
# Labels to get segmentation
LABEL_IDS=(BKG        CA1 CA2 DG CA3 Tail SUB ERC BA35 BA36 PHC CS)
LABEL_MRG=("0 6 7 13" 1   2   3  4   5    8   9   10   11   12  14)
...
VOLRANGES=("1 2 3 4 5 8"  1   2   4   "1 2 4" 3  8   5    9   10   11   12  6           14 7     13)
...
function main()
{
  ...
  if [[ 1 == 0 ]]; then
  for method in MV SVWV ashs_heur ashs_usegray UNET; do
...
{
  mkdir -p $TEMPLATEDIR
  rm -rf $TEMPLATEDIR/manuifest.txt
  ...
  for ((i=0;i<4;i++)); do 
    ...
    for row in $(cat $testtxt); do

      id=$(echo $row | cut -d , -f 1)
      echo "$id \
            $(ls $RAWDATADIR/$id/*mprage.nii.gz) \
            $(ls $RAWDATADIR/$id/*tse.nii.gz) \
            $(ls $RAWDATADIR/$id/*_left_finalLW.nii.gz) \
            $(ls $RAWDATADIR/$id/*_right_finalLW.nii.gz)" \
...
function warp_errormap_subj()
{
  ...
  if [[ $method == "MV" ]]; then
    ...
    AUTOSEG=$(ls /home/longxie/DeepLabelFusion/PMC_Prisma_3T_withflip/exp_ASHST1/exp*/testing/$id/multiatlas/fusion/lfseg_corr_usegray_${side}.nii.gz)
  elif [[ $method == "ashs_heur" ]]; then
    AUTOSEG=$(ls /home/longxie/DeepLabelFusion/PMC_Prisma_3T_withflip/exp_ASHST1/exp*/testing/$id/multiatlas/fusion/lfseg_heur_${side}.nii.gz)
...
function get_volume()
{
  ...
  for id in ${IDs[*]}; do
    ...
    for side in left right; do
      ...
      REFSEG=$RAWDATADIR/$id/${id}_${side}_finalLW.nii.gz
