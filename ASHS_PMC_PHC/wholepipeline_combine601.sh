...
ASHSTEMPDIR=$ROOT/OriginASHS/headtailatlas/run02/final/template
...
function pairwise_sub()
{
  ...
  for side_mov in left right; do
    ...
    if [[ $idside_mov != $idside_fix ]]; then
      ...
      if [[ -f $OUTDIR/${fnside_mov}_to_${fnside_fix}_reslice_seg.nii.gz ]]; then
        ...
        c3d $(for sub in ${LABEL_IDS[*]}; do echo $TMPDIR/${fnside_mov}_to_${fnside_fix}_reslice_${sub}.nii.gz; done) \
          -vote -type ushort \
          -o $OUTDIR/${fnside_mov}_to_${fnside_fix}_reslice_seg.nii.gz
...
function completeness
{
  ...
  for side_fix in left right; do
    ...
    for id_fix in $IDS; do
      ...
      if [[ -f $OUTDIR/${idside_mov}_to_${idside_fix}_reslice_seg.nii.gz ]]; then
          echo "ok"
      else
...
function similarity()
{
  ...
  rm -rf ${SIM_DIR}/sim_${SIM_TYPE}.txt

  for side in left right; do
  for id_fix in $IDS; do

      cat ${SIM_DIR}/tmp/${id_fix}_${side}.txt \
          >> ${SIM_DIR}/sim_${SIM_TYPE}.txt
...
function similarity_sub()
{
  ...
  fn_out=$SIM_DIR/tmp/${idside_fix}.txt
  ...
  for id_mov in $IDS; do
    ...
    if [[ $idside_mov == $idside_fix ]]; then
      ...
      OVL_tmp=$(c3d $PWDIR/${fnside_fix}/${fnside_mov}_to_${fnside_fix}/${fnside_mov}_to_${fnside_fix}_reslice_seg.nii.gz \
          -replace 1 0 4 0 6 0 \
...
function clustering_sub()
{
  ...
  source $PKGDIR/matlab_batch.sh \
     $TMPDIR/${id}/ \
     $MATLABCODEDIR \
     clustering_ProbPCACombine_${expid} \
     ${SIM_DIR}/sim_${SIM_TYPE}.txt \
     $GTGROUPDIR/group_left_200.txt \
     $GTGROUPDIR/group_right_200.txt \
     $SUBJ_TXT \
     $INITGRP_DIR
}
...
function initialization()
{
  ...
  INIT_ID=($(cat $INITGRP_DIR/AutoCenters.txt))
  for grp in ${groups[*]}; do

    i=$((grp-1))
    INIT=${INIT_ID[i]}
    id=$(echo $INIT | cut -d '_' -f 1)
    side=$(echo $INIT | cut -d '_' -f 2)
...
function warp_labels_subj()
{
  ...
  for sub in ${LABEL_IDS[*]}; do
    ...
    antsApplyTransforms -d 3 \
      -r $ASHSRUNDIR/${id}/final/${id}_${side}_lfseg_corr_usegray_cleanup_dividedCS.nii.gz \
  ...
  for ((i=0;i<${#LABEL_IDS[*]};i++)); do
    c3d $ASHSRUNDIR/${id}/final/${id}_${side}_lfseg_corr_usegray_cleanup_dividedCS.nii.gz \
...
function VTMeshSubSample_sub()
{
  ...
  ITDIR=$WORKGRPDIR/$(printf iter_%02d $((ITER-1)) )
  ...
  for ((i=0;i<${#EXTRAMESHES[*]};i++)); do
    ...
    vtklevelset \
      $ITDIR/template_${grp}_${EXTRAMESHES[i]}.nii.gz \
      $MESHGRPDIR/template_${grp}_${EXTRAMESHES[i]}.stl \
      $template_th

  done

  cp $MESHGRPDIR/template*.stl \
     $MESHGRPDIR/template/

  # perform subsampling
  SAM=$MESHGRPDIR/template/template_${grp}_MRG_sampled.ply
  /data/picsl-build/pauly/vcg/gcc64rel/mesh_poisson_sample \
    $MESHGRPDIR/template/template_${grp}_MRG.stl \
    $SAM 2000

  # comvert into vtk mesh
  TEMPLATE=$MESHGRPDIR/template/template_${grp}_MRG_sampled.vtk
...
function VTGeodesicShoot_sub()
{
  idside=$1
  grp=$2
  id=$(echo $idside | cut -d '_' -f 1)
  side=$(echo $idside | cut -d '_' -f 2)
  id=$(ls $ASHSRUNDIR | grep $id)
  MESHGRPDIR=$INITTEMP_DIR/group${grp}/meshwarp
  WORKGRPDIR=$INITTEMP_DIR/group${grp}/work
  LABELWARPDIR=$INITTEMP_DIR/group${grp}/labelwarp
  ...
  $ANTSPATH/ComposeMultiTransform 3 \
     $TMPDIR/compose.nii \
     -R $WORKGRPDIR/template_${grp}_tse.nii.gz \
  ...
  greedy -d 3 \
    ...
    -rs $MESHGRPDIR/template/template_${grp}_MRG_sampled.vtk \
        $SUBJDIR/${id}_${side}_${grp}_MRG_tempfit.vtk \
    -r $TMPDIR/compose.nii

  # use procrustes to align the two mesh
  vtkprocrustes $SUBJDIR/${id}_${side}_${grp}_MRG_tempfit.vtk \
    $MESHGRPDIR/template/template_${grp}_MRG_sampled.vtk \
  ...
  greedy -d 3 \
    ...
    -r $SUBJDIR/template_to_${id}_${side}_${grp}_rigid.mat

  # perform geodesic shooting
  /home/cwang/pcmrep/PointSetGeodesicShooting_CUDA_c00/lmshoot_cuda -d 3 \
    -m $MESHGRPDIR/template/template_${grp}_MRG_sampled.vtk \
       $SUBJDIR/${id}_${side}_${grp}_MRG_tempfit_rigid.vtk \
...
function VTGShootAvg_sub()
{
  ...
  avgmesharr \
    $VTGEOSHOOTDIR/*/*_${grp}_momentum.vtk \
    InitialMomentum \
    $MESHGRPDIR/template/template_${grp}_MRG_sampled.vtk \
    $VTGEOSHOOTDIR/shavg/template_${grp}_MRG_average_momenta.vtk

  # Perform the shooting and generate warp
  lmtowarp -d 3 -n 40 \
    -r $WORKGRPDIR/template_${grp}_seg.nii.gz \
    -m $VTGEOSHOOTDIR/shavg/template_${grp}_MRG_average_momenta.vtk \
    -o $VTGEOSHOOTDIR/shavg/template_${grp}_MRG_average_momenta_warp.nii.gz \
    -s 2.0

  # Apply the warp to the template
  greedy -d 3 \
    -rf $WORKGRPDIR/template_${grp}_seg.nii.gz \
    -rs $MESHGRPDIR/template_${grp}_MRG.vtk \
        $VTGEOSHOOTDIR/shavg/template_${grp}_MRG_Gshoot.vtk \
    -r $VTGEOSHOOTDIR/shavg/template_${grp}_MRG_average_momenta_warp.nii.gz
}
...
function VTShapeStats_sub()
{
  ...
  LABELWARPDIR=$INITTEMP_DIR/group${grp}/labelwarp
  VTGEOSHOOTDIR=$INITTEMP_DIR/group${grp}/VTGeoShoot
  PCADIR=$VTGEOSHOOTDIR/shavg/pca/
  rm -rf $PCADIR
  mkdir -p $PCADIR
  GRP_IDS=$(cat $INITGRP_DIR/subjID_${grp}.txt)
  ...
  idx=1
  echo "ID" > $PCADIR/IDs_${grp}.csv
  for idside in $GRP_IDS; do
    id=$(echo $idside | cut -d '_' -f 1)
    side=$(echo $idside | cut -d '_' -f 2)
    id=$(ls $ASHSRUNDIR | grep $id)
    local MOMENTS=$VTGEOSHOOTDIR/$id/${id}_${side}_${grp}_momentum.vtk
    echo $(echo $idx) $(dumpmeshattr $MOMENTS InitialMomentum) >> $PCADIR/initial_momenta_${grp}.txt
    echo $idx >> $PCADIR/IDs_${grp}.csv
    idx=$((idx+1))
  done

  # Generate an array of mesh coordinates for R
  dumpmeshpoints $VTGEOSHOOTDIR/shavg/template_${grp}_MRG_average_momenta.vtk | tail -n +3 > $PCADIR/template_points_${grp}.txt

  # Run the R statistics notebook
  Rscript run_pca.R \
    $PCADIR/IDs_${grp}.csv \
    $PCADIR/initial_momenta_${grp}.txt \
    $PCADIR/template_points_${grp}.txt \
    $PCADIR
}
  ...
  for grp in ${groups[*]}; do
    for what in mode1 mode2 mode3 mode4 mode5; do
      ...
      qsub -cwd -o $DUMPDIR -j y \
           -q all.q,basic.q \
           -l h_vmem=4.1G,s_vmem=4G \
           -N "${PREFIX}_${grp}_${what}" $0 \
           VTMakeMovies_sub $grp $what
      sleep 0.1
...
function VTMakeMovies_sub()
{
  ...
  # Template
  local TEMPMESH=$MESHGRPDIR/analysis/labels/thick_left_${grp}_MRG.vtk
...
function Insertion_sub()
{
  ...
  if [[ $id_mov != $id_fix ]]; then
    ...
    c3d $(for sub in ${LABEL_IDS[*]}; do echo $TMPDIR/${id_mov}_to_${id_fix}_reslice_${sub}.nii.gz; done) \
        -vote -type ushort \
        -o $OUTDIR/${id_mov}_to_${id_fix}_reslice_seg.nii.gz
  ...
  for side in left right; do
    for id_fix in $IDS; do
      ...
      if [[ $id_fix == "template1" ]] || \
        ...
        idside_fix=${id_fix}_${side}
      fi

      cat ${SIM_DIR}/tmp/${idside_fix}.txt \
          >> ${SIM_DIR}/sim_${SIM_TYPE}_withtemp.txt
...
function PropagateTemplates()
{
  ...
  #for grp_fix in 1; do
    ...
    qsub -cwd -o $DUMPDIR -j y \
      ...
      PropagateTemplates_sub $grp_fix $grp_mov
    sleep 0.1
...
##########################################################
  ...
  c3d $(for sub in ${LABEL_IDS[*]}; do echo $DATADIR/template${grp_mov}_${sub}.nii.gz; done) \
    -vote -type ushort -as SEG \
    $DATADIR/template${grp_fix}_tse.nii.gz \
    -push SEG -interp 0 -reslice-identity \
    -o $TRANSITERDIR/template${grp_mov}_seg.nii.gz
  for ((ii=0;ii<${#EXTRAMESHES[*]};ii++)); do
    ln -sf $INITTEMP_DIR/group${grp_mov}/meshwarp/analysis/labels/thick_${grp_mov}_${EXTRAMESHES[ii]}.vtk $TRANSITERDIR/${i}_template${grp_mov}_${EXTRAMESHES[ii]}.vtk
  done
#########################################################
  warps=""   
  invwarps=""
  ...
  while true; do
    ...
    # perform registration to get warps if necessary
    if [ ! -f $PWDIR/${idside_to}/${idside_from}_to_${idside_to}/${idside_from}_to_${idside_to}_pairwiseWarp.nii.gz ] && \
       [ ! -f $PWDIR/${idside_to}/${idside_from}_to_${idside_to}/${idside_from}_to_${idside_to}_pairwiseInverseWarp.nii.gz ]; then

      OUTDIR=$PWDIR/${idside_to}/${idside_from}_to_${idside_to}
      mkdir -p $OUTDIR

      # Use ml_affine for nice affine alignment
      /data/picsl/pauly/wolk/ashs/ext/Linux/bin/ml_affine \
        $DATADIR/${idside_to}_seg.nii.gz \
        $DATADIR/${idside_from}_seg.nii.gz \
        $OUTDIR/${idside_from}_to_${idside_to}_mlaffine.txt

      # Convert that to ITK format
      c3d_affine_tool \
        $OUTDIR/${idside_from}_to_${idside_to}_mlaffine.txt \
        -oitk \
        $OUTDIR/${idside_from}_to_${idside_to}_mlaffine_itk.txt

      CMD=""
      for sub in ${REG_LABELS_1[*]}; do
        CMD="$CMD -m MSQ[$DATADIR/${idside_to}_${sub}.nii.gz,$DATADIR/${idside_from}_${sub}.nii.gz,$WGT_1]"
      done

      if [[ $ANTs_x_1 == "Y" ]]; then
        c3d $DATADIR/${idside_to}_seg.nii.gz -dup \
            $DATADIR/${idside_from}_seg.nii.gz \
            -int 0 -reslice-identity \
            -add -binarize -dilate 1 10x10x10vox \
            -o $OUTDIR/${idside_from}_to_${idside_to}_mask.nii.gz
        ANTs_mask_1="-x $OUTDIR/${idside_from}_to_${idside_to}_mask.nii.gz"
      else
        ANTs_mask_1=""
      fi
      ...
      ANTS 3 $CMD \
         ...
         $ANTs_G_1 \
         $ANTs_mask_1 \
         -a $OUTDIR/${idside_from}_to_${idside_to}_mlaffine_itk.txt \
         --continue-affine 0 \
         $ANTs_all_metrics_1 \
         -o $OUTDIR/${idside_from}_to_${idside_to}_pairwise.nii.gz

    fi

    # get warps
    warps="$PWDIR/${idside_to}/${idside_from}_to_${idside_to}/${idside_from}_to_${idside_to}_pairwiseWarp.nii.gz $PWDIR/${idside_to}/${idside_from}_to_${idside_to}/${idside_from}_to_${idside_to}_mlaffine_itk.txt $warps"

    invwarps="$invwarps -i $PWDIR/${idside_to}/${idside_from}_to_${idside_to}/${idside_from}_to_${idside_to}_mlaffine_itk.txt $PWDIR/${idside_to}/${idside_from}_to_${idside_to}/${idside_from}_to_${idside_to}_pairwiseInverseWarp.nii.gz"

########################################################
  if [[ 1 == 1 ]]; then
    # compose transform
    TRANSITERDIR=$TRANSDIR/${grp_mov}_to_${grp_fix}/${i}_${idside_from}_to_${idside_to}
    TRANSITERTMPDIR=$TRANSITERDIR/tmp
    mkdir -p $TRANSITERTMPDIR

    # link the original intermediate template
    c3d $(for sub in ${LABEL_IDS[*]}; do echo $DATADIR/${idside_to}_${sub}.nii.gz; done) \
    -vote -type ushort -as SEG \
    $DATADIR/template${grp_fix}_tse.nii.gz \
    -push SEG -interp 0 -reslice-identity \
    -o $TRANSITERDIR/${idside_to}_seg.nii.gz

    # warp template 
    $ANTSPATH/ComposeMultiTransform 3 \
      $TRANSITERTMPDIR/template${grp_mov}_InitWarp.nii.gz \
    ...
    for sub in $KINDS; do
      ...
      WarpImageMultiTransform 3 \
        ...
        $TRANSITERTMPDIR/template${grp_mov}_inittotemp_reslice_${sub}.nii.gz \
        -R $DATADIR/template${grp_fix}_tse.nii.gz \
        $TRANSITERTMPDIR/template${grp_mov}_InitWarp.nii.gz
    ...
    # Create the segmentation for the template
    c3d $(for sub in ${LABEL_IDS[*]}; do echo $TRANSITERTMPDIR/template${grp_mov}_inittotemp_reslice_${sub}.nii.gz; done) \
      -vote -type ushort \
      -o $TRANSITERDIR/template${grp_mov}_inittotemp_reslice_seg.nii.gz

    # generate a surface mesh
    for ((ii=0;ii<${#EXTRAMESHES[*]};ii++)); do
      LABELDEF=(${EXTRAMESHESDEF[ii]})
      c3d $(for ((j=0;j<${#LABEL_IDS[*]};j++)); do echo "$TRANSITERTMPDIR/template${grp_mov}_inittotemp_reslice_${LABEL_IDS[j]}.nii.gz -scale ${LABELDEF[j]}"; done) \
        -accum -add -endaccum \
        -o $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}.nii.gz

      vtklevelset \
        $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}.nii.gz \
        $TRANSITERDIR/${i}_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
        $template_th
      ...
      for ((jj=0;jj<${#LABEL_IDS[*]};jj++)); do
        if [[ ${LABELDEF[$jj]} == "1" ]]; then
          ...
          mesh_image_sample \
            $TRANSITERDIR/${i}_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
            $TRANSITERTMPDIR/template${grp_mov}_inittotemp_reslice_${LABEL_IDS[jj]}.nii.gz \
            $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}_${sub}.vtk \
            PROB
          MESHES="$MESHES $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}_${sub}.vtk"
        fi
      done

      # merge prob arrays
      mesh_merge_arrays \
        -r $TRANSITERDIR/${i}_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
        $TRANSITERDIR/${i}_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
        PROB $MESHES
      ...
      $MATLAB_BIN -nojvm -nosplash -nodesktop <<-MATCODE
        addpath('/data/picsl/longxie/ASHS/thickness_newlabel/matlabcode');
        compute_mesh_label('$TRANSITERDIR/${i}_template${grp_mov}_${EXTRAMESHES[$ii]}.vtk');
MATCODE
...
##########################################################
  if [[ 1 == 1 ]]; then
  TRANSITERDIR=$TRANSDIR/${grp_mov}_to_${grp_fix}/target_template${grp_fix}/
  mkdir -p $TRANSITERDIR
  ln -sf $DATADIR/template${grp_fix}_seg.nii.gz \
         $TRANSITERDIR/template${grp_fix}_seg.nii.gz
  for ((ii=0;ii<${#EXTRAMESHES[*]};ii++)); do
    ln -sf $INITTEMP_DIR/group${grp_fix}/meshwarp/analysis/labels/thick_${grp_fix}_${EXTRAMESHES[ii]}.vtk $TRANSITERDIR/${i}_template${grp_fix}_${EXTRAMESHES[ii]}.vtk
  done
  fi

  if [[ 0 == 1 ]]; then
  # link the direct registration result from pairwise registration
  TRANSITERDIR=$TRANSDIR/${grp_mov}_to_${grp_fix}/direct/
  TRANSITERTMPDIR=$TRANSITERDIR/tmp
  mkdir -p $TRANSITERTMPDIR
  for ((ii=0;ii<${#EXTRAMESHES[*]};ii++)); do
    LABELDEF=(${EXTRAMESHESDEF[ii]})
    c3d $(for ((j=0;j<${#LABEL_IDS[*]};j++)); do echo "$PWDIR/template${grp_fix}/template${grp_mov}_to_template${grp_fix}/template${grp_mov}_to_template${grp_fix}_reslice_${LABEL_IDS[j]}.nii.gz -scale ${LABELDEF[j]}"; done) \
      -accum -add -endaccum \
      -o $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}.nii.gz

    vtklevelset \
      $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}.nii.gz \
      $TRANSITERDIR/direct_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
      $template_th
    ...
    for ((jj=0;jj<${#LABEL_IDS[*]};jj++)); do
      if [[ ${LABELDEF[$jj]} == "1" ]]; then
        ...
        mesh_image_sample \
          $TRANSITERDIR/direct_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
          $PWDIR/template${grp_fix}/template${grp_mov}_to_template${grp_fix}/template${grp_mov}_to_template${grp_fix}_reslice_${LABEL_IDS[jj]}.nii.gz \
          $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}_${sub}.vtk \
          PROB
        MESHES="$MESHES $TRANSITERTMPDIR/template${grp_mov}_${EXTRAMESHES[ii]}_${sub}.vtk"
      fi
    ...
    # merge prob arrays
    mesh_merge_arrays \
      -r $TRANSITERDIR/direct_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
      $TRANSITERDIR/direct_template${grp_mov}_${EXTRAMESHES[ii]}.vtk \
      PROB $MESHES
      ...
      $MATLAB_BIN -nojvm -nosplash -nodesktop <<-MATCODE
        addpath('/data/picsl/longxie/ASHS/thickness_newlabel/matlabcode');
        compute_mesh_label('$TRANSITERDIR/direct_template${grp_mov}_${EXTRAMESHES[$ii]}.vtk');
MATCODE
