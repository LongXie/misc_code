...
function warp_labels_subj()
{
  ...
  for ((i=0;i<${#LABEL_IDS[*]};i++)); do
    rm ${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_${LABEL_IDS[i]}_ashs.nii.gz
    rm ${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_${LABEL_IDS[i]}_tempfit.nii.gz
  done
  ...
  if [[ -d $ASHSRUNDIR/$id/refseg ]]; then
    for ((i=0;i<${#LABEL_IDS[*]};i++)); do
      c3d $ASHSRUNDIR/${id}/refseg/refseg_${side}.nii.gz \
        -replace $(for k in ${LABEL_MRG[i]}; do echo $k 999; done) \
        -thresh 999 999 1 0 \
        -o ${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_${LABEL_IDS[i]}_refseg.nii.gz
    done

    c3d $(for sub in ${LABEL_IDS[*]}; do echo ${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_${sub}_refseg.nii.gz; done) -vote -type ushort \
      -o ${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_seg_refseg.nii.gz

    for ((i=0;i<${#LABEL_IDS[*]};i++)); do
      rm ${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_${LABEL_IDS[i]}_refseg.nii.gz
    done
  fi
}
...
function evaluation()
{
  ...
  for side in left right; do
    ...
    echo "ID ${EVALLABELS[*]} ${MESH_EVAL[*]}" \
      > $EVALDIR/overlap_refseg_${side}.txt
    ...
    for ids in ${IDs[*]}; do
      ...
      if [[ -f $EVALTMPDIR/${id}_${side}_refseg_overlap.txt ]]; then
        cat $EVALTMPDIR/${id}_${side}_refseg_overlap.txt \
          >> $EVALDIR/overlap_refseg_${side}.txt
      fi
...
function eval_subj()
{
  ...
  FITTED=${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_seg_tempfit_ref.nii.gz
  REFSEG=${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_seg_refseg.nii.gz
  if [[ -f $REFSEG ]]; then
  # compute dice
  c3d ${expdir}/group${grp}/labelwarp/${id}_${side}_${grp}_seg_tempfit.nii.gz -replace 5 6 -o $FITTED
    ...
    #for side in left right; do
     ...
     for design in $(ls $STATDIR | grep "design_.*txt" | grep $side | grep $grp ); do
        ...
        for ((igrp=0;igrp<${#ANGRP[*]};igrp++)); do
          ...
          # Submit job for this subject
          qsub -cwd -o $DUMPDIR -j y \
            -N "${PREFIX}_${exp}_${grp}_${GRPNM[igrp]}" \
            $0 thick_stats_sub $exp $grp $igrp
  ...
  for side in left right; do
    ...
    for sub in $MYGRP; do
      ...
      MESHES=$(for id in $SUBJ; do \
        echo $(find ${expdir}/group${grp}/meshwarp | grep ${id} | grep ${side}_${grp}_${sub}_thickmap.vtk); done)
...
function mean_thickness()
{
  PREFIX=MT${expid}
  for grp in ${groups[*]}; do
    MESHGRPDIR=${expdir}/group${grp}/meshwarp
  ...
  IDS=($(cat $SUBJ_TXT))
  rm -rf ${expdir}/analysis
  mkdir -p ${expdir}/analysis
  ...
  echo "$header" > ${expdir}/analysis/summary_thick.csv
  ...
  for ((i=0;i<${#IDS[*]};i++)); do
    ...
    # load left side
    side=left
    GRP=($(cat $WORKDIR/group/group_${side}_${expid}.txt))
    grp=${GRP[$i]}
    THICK_LEFT=$(cat ${expdir}/group${grp}/meshwarp/analysis/labels/thick_${side}_${grp}_MRG.txt | grep $id)

    # load right side
    side=right
    GRP=($(cat $WORKDIR/group/group_${side}_${expid}.txt))
    grp=${GRP[$i]}
    THICK_RIGHT=$(cat ${expdir}/group${grp}/meshwarp/analysis/labels/thick_${side}_${grp}_MRG.txt | grep $id | cut -d ' ' -f 2)

    echo "${THICK_LEFT}${THICK_RIGHT}" >> \
      ${expdir}/analysis/summary_thick.csv
  ...
  Rscript $WORKDIR/scripts/summary_thickness_analysis.R \
    ...
    ${expdir}/analysis/summary_thick.csv \
    ${expdir}/analysis/mean_thickness_stats.csv
}
...
function mean_thickness_sub()
{
  ...
  MESHGRPDIR=${expdir}/group${grp}/meshwarp
  WORKGRPDIR=${expdir}/group${grp}/work
