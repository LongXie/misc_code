...
function copy_data()
{
  ...
  # ln left regmask
  ln -sf $ASHSTEMPDIR/tse_to_chunktemp_left_regmask.nii.gz \
    $DATADIR/tempreg/tse_to_chunktemp_left_regmask.nii.gz

  # flip right regmask
  c3d $ASHSTEMPDIR/tse_to_chunktemp_right_regmask.nii.gz \
     -flip x \
    -o $DATADIR/tempreg/tse_to_chunktemp_right_regmask_flip.nii.gz

  # register fliped right to left
  ml_affine \
    $DATADIR/tempreg/tse_to_chunktemp_left_regmask.nii.gz \
    $DATADIR/tempreg/tse_to_chunktemp_right_regmask_flip.nii.gz \
  ...
  WarpImageMultiTransform 3 \
    $DATADIR/tempreg/tse_to_chunktemp_right_regmask_flip.nii.gz \
    $DATADIR/tempreg/tse_to_chunktemp_right_regmask_flip_trans.nii.gz \
    -R $DATADIR/tempreg/tse_to_chunktemp_right_regmask_flip.nii.gz \
