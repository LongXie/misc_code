...
FIXEDSEGDIR=$ROOT/SuperResolution/ASHSexp/exp002/fullset/fixed_seg
...
function copy_subject()
{
  ...
  # ASHS segmentation
  SEG=$FIXEDSEGDIR/${fn}_${side}_lfseg_corr_usegray_dividedCS.nii.gz
...
function warp_labels_subj()
{
  ...
  for sub in ${LABEL_IDS[*]}; do
    ...
    antsApplyTransforms -d 3 \
      -r $FIXEDSEGDIR/${id}_${side}_lfseg_corr_usegray_dividedCS.nii.gz \
  ...
  for ((i=0;i<${#LABEL_IDS[*]};i++)); do
    c3d $FIXEDSEGDIR/${id}_${side}_lfseg_corr_usegray_dividedCS.nii.gz \
